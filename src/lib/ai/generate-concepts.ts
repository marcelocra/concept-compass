/**
 * Shared AI concept generation logic
...
/**
 * Generate related concepts for a given input concept using OpenRouter AI
 * This function can be called directly from server-side code without HTTP overhead
 */
export async function generateConcepts(concept: string): Promise<GenerateConceptsResult> {
  try {
    ...
    const sanitizedConcept = sanitizeConcept(concept);
    if (!sanitizedConcept) {
      return { success: false, error: "Concept cannot be empty after sanitization" };
    }
    ...
    if (!openRouterResponse || !openRouterResponse.ok) {
      ...
      if (openRouterResponse?.status === 429) {
        return { success: false, error: "AI service is currently overloaded. Please try again in a moment." };
      }
      ...
      return { success: false, error: "AI service is temporarily unavailable. Please try again." };
    }

    // Use the data we already parsed and validated in the retry loop
    if (!openRouterData) {
      console.error("No valid response data after retries");
      return { success: false, error: "AI service failed to provide valid response after retries." };
    }
    ...
    console.error("Unexpected error in generateConcepts:", error);
