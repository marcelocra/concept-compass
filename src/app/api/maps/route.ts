...
const createMapSchema = z.object({
  ...
  initialConcept: z.string().min(1, "Initial concept is required").max(200, "Initial concept must be 200 characters or less"),
});
...
export async function POST(request: NextRequest): Promise<NextResponse<CreateMapResponse>> {
  try {
    ...
    const [newMindMap] = await db.insert(mindMaps).values({
      userId,
      name,
      graphData,
    }).returning();
